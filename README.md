...
Git City(thegitcity.com)를 벤치마크한 프로젝트로, GitHub 커밋 활동을 3D 빌딩으로 시각화하는 Git City의 컨셉을 AWS 인프라에 적용했습니다. AWS CloudTrail 로그를 수집·집계하여 계정의 API 활동을 **3D 복셀 구름 섬(라퓨타 스타일)**으로 시각화하는 웹 플랫폼입니다.

- API 호출 수 → 구름 두께, 리소스 수 → 구름 면적, 에러율 → 빨간 파티클로 매핑
- AWS 공식 카테고리 색상 7개로 서비스 종류를 직관적으로 구분
- 세 가지 모드: **시뮬레이터** (슬라이더) / **프리셋** (샘플 5종) / **실제 AWS 연결**
